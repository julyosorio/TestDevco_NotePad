...
public class ScheduledTrxOwnAccountsStepDefinitions {
private InfoTrx infoTrx;

    @When("{string} makes a scheduled transaction with own accounts")
    public void makesAScheduledTransactionWithOwnAccounts(String actor, InfoTrx infoTrx) {
    ...
    @Then("{string} verifies that it displays the transaction successful message for proprietary accounts")
    public void verifiesThatItDisplaysTheTransactionSuccessfulMessageForProprietaryAccounts(String actor) {
        OnStage.theActorInTheSpotlight().attemptsTo(
    ...
    @When("{string} searches the scheduled transactions table for the transaction between own accounts with the data")
    public void searchesTheScheduledTransactionsTableForTheTransactionBetweenOwnAccountsWithTheData(String actor, InfoTrx infoTrx) {
        this.infoTrx=infoTrx;
        OnStage.theActorInTheSpotlight().attemptsTo(
                MenuPendingTransactions.goToTheMenuProgrammed(TYPE_OPERATION_TRANSFER),
                ScheduledTransactions.applyFilters(infoTrx, OWN_ACCOUNTS)
        );
    }

    @Then("{string} validate that the information of the scheduled transaction with own accounts table register was correct")
    public void validateThatTheInformationOfTheScheduledTransactionWithOwnAccountsTableRegisterWasCorrect(String actor) {
        OnStage.theActorInTheSpotlight().attemptsTo(
                Ensure.that(COL_TRANSACTIONTYPE).text().contains(OWN_ACCOUNTS),
    ...
    @When("{string} goes to summary page of current transaction between own accounts")
    public void goesToSummaryPageOfCurrentTransactionBetweenOwnAccounts(String actor, InfoTrx infoTrx) {
        this.infoTrx = infoTrx;
        OnStage.theActorInTheSpotlight().attemptsTo(
                TransfersOfOwnAccounts.goTo(infoTrx),
                SelectIrisAccounts.forTransaction(infoTrx),
    ...
    @Then("{string} verifies for scheduled transaction between own accounts that information of summary page be equal to the entered information")
    public void verifiesForScheduledTransactionBetweenOwnAccountsThatInformationOfSummaryPageBeEqualToTheEnteredInformation(String actor) {
        OnStage.theActorInTheSpotlight().attemptsTo(
                ...
                Ensure.that(LABEL_DESTINATION_OWN_ACCOUNTS).text().contains(infoTrx.getDestinationAccountNumber())
    ...
    @When("{string} searches the table of scheduled transactions to view the transaction voucher between own accounts")
    public void searchesTheTableOfScheduledTransactionsToViewTheTransactionVoucherBetweenOwnAccounts(String actor, InfoTrx infoTrx) {
        this.infoTrx=infoTrx;
        OnStage.theActorInTheSpotlight().attemptsTo(
                MenuPendingTransactions.goToTheMenuProgrammed(TYPE_OPERATION_TRANSFER),
                ScheduledTransactions.applyFilters(infoTrx, OWN_ACCOUNTS),
                SelectARegisterCorrect.scheduledTransactionTable(infoTrx, OWN_ACCOUNTS)
        );
    }

    @Then("{string} verifies that the scheduled transaction voucher information is correct between own accounts")
    public void verifiesThatTheScheduledTransactionVoucherInformationIsCorrectBetweenOwnAccounts(String actor) {
        OnStage.theActorInTheSpotlight().attemptsTo(
                Ensure.that(LABEL_TITLE).text().contains("Transferencia - Cuentas propias"),
    ...
    @When("{string} deletes a scheduled transaction between own accounts from the detail of a selected record in the scheduled transactions table")
    public void deletesAScheduledTransactionBetweenOwnAccountsFromTheDetailOfASelectedRecordInTheScheduledTransactionsTable(String actor, InfoTrx infoTrx) {
        this.infoTrx= infoTrx;
        OnStage.theActorInTheSpotlight().attemptsTo(
                MenuPendingTransactions.goToTheMenuProgrammed(TYPE_OPERATION_TRANSFER),
                ScheduledTransactions.applyFilters(infoTrx, OWN_ACCOUNTS),
    ...
    @Then("{string} verifies the successful deletion message of a scheduled transaction between own accounts")
    public void verifiesTheSuccessfulDeletionMessageOfAScheduledTransactionBetweenOwnAccounts(String actor) {
        OnStage.theActorInTheSpotlight().attemptsTo(
